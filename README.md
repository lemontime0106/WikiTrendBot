...
1. 키워드 또는 퍼플렉시티 표 전체를 입력받습니다.
2. 우선순위가 가장 높은 행을 고르고 웹 검색 결과를 자동으로 조사합니다.
3. 사이트 핵심 분야, 위험도, 직접 경험 필요 여부를 먼저 판정합니다.
4. 근거가 있는 주장과 독자 결과를 포함한 기획안을 구조화해 만듭니다.
5. 2,200~4,000자 수준의 글을 작성하고, 자동 검색에서 실제 URL을 찾은 경우에만 링크를 넣습니다.
...
- 웹 화면에서는 키워드 한 개 또는 퍼플렉시티의 표 전체를 붙여 넣습니다.
- 표를 붙이면 `우선순위`가 가장 높은 행을 고르고 검색 의도, 주요 독자, 추천 제목, 핵심 내용을 자동 반영합니다.
...
API의 `user_purpose`, `firsthand_notes`는 외부 자동화 연동이나 운영자가 실제 검증 자료를 보유한 경우에만 사용하는 선택 필드입니다. 신뢰 URL 입력은 요구하지 않습니다. AI가 실제로 하지 않은 경험을 `firsthand_notes`에 생성해서는 안 됩니다.
...
- `CONTENT_MAX_RESEARCH_CONTEXT_CHARS`: LLM에 전달할 검색 자료 최대 길이, 기본 2600
- `CONTENT_MAX_RESEARCH_SOURCES_IN_PROMPT`: 프롬프트에 포함할 자료 수, 기본 4
- `CONTENT_MAX_PLANNING_BRIEF_CHARS`: 붙여넣은 기획 중 사용할 최대 길이, 기본 1200
...
규칙 기반 검사는 H1 1개, H2 3개 이상, 허위 URL, 직접 경험 근거, 양산형 고정 문구, 고위험 주제, 기존 글 중복, 이미지 슬롯 수를 검사합니다. 링크 개수와 참고자료 구역은 발행 차단 조건이 아닙니다.
