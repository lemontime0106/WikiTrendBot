...
- `사용자 목적`: 독자가 글을 읽고 내릴 판단이나 완료할 작업을 적습니다.
- `직접 경험·검증 메모`: 사용 환경, 제품 버전, 기간, 측정 방법, 결과, 실패 사례를 적습니다. 리뷰·추천·직접 비교 글은 80자 이상이 필요합니다.
- `추가 공식 출처 URL`: 제품 공식 문서, 회사 발표, 정부·기관 자료처럼 원문 확인이 가능한 URL을 한 줄에 하나씩 넣습니다.
...
```json
{
  ...
  "user_purpose": "작은 SaaS 팀이 비용을 줄이기 전에 확인할 측정 기준을 정리",
  "firsthand_notes": "2026년 7월 테스트 환경과 측정 방식, 실제 요청량, 변경 전후 결과를 구체적으로 기록...",
  "source_urls": [
